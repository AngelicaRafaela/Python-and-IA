...
def log_transacao(funcao):
    ...
    def wrapper(*args, **kwargs):
        # Registrar data, hora e tipo de transação antes de executar a função
        print(f"[{datetime.now()}] Transação do tipo: {funcao.__name__}")
...
class Historico:
    ...
    def transacoes_do_dia(self):
        ...
        transacoes = []
        for transacao in self._transacoes:
            formatos_data = ["%d-%m-%Y %H:%M:%S", "%d-%m-%Y %H:%M"]
            for formato in formatos_data:
                try:
                    # Tentar analisar a data usando o formato atual
                    data_transacao = datetime.strptime(transacao["data"], formato).date()
                    if data_atual == data_transacao:
                        transacoes.append(transacao)
                    break  # Parar o loop se a data for analisada com sucesso
                except ValueError:
                    # Se não for possível analisar a data com o formato atual, continue tentando outros formatos
                    continue
        return transacoes
